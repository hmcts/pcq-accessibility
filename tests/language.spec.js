...
test.beforeEach(async ({ page }) => {
  language = new pcqPages(page);
  axe = new AxeUtils(page);
  await language.invokerSetUp();
  await language.clickInvoke();
  await page.goto('/language');
});

describe('@pcq @language', () => {
  test('@axe language page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await language.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.languagePageH1, 'i'));
  });

  test('@checklist 6. Page Title is unique', async ({}) => {
    await language.pageTitleUnique();
  });
