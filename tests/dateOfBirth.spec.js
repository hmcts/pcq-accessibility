...
test.beforeEach(async ({ page }) => {
  dOB = new pcqPages(page);
  axe = new AxeUtils(page);
  await dOB.invokerSetUp();
  await dOB.clickInvoke();
  await page.goto('/date-of-birth');
});

describe('@pcq @dOB', () => {
  test('@axe dOB page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await dOB.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.dOBPageH1, 'i'));
  });
