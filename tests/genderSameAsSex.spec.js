...
test.beforeEach(async ({ page }) => {
  gender = new pcqPages(page);
  axe = new AxeUtils(page);
  await gender.invokerSetUp();
  await gender.clickInvoke();
  await page.goto('/gender-same-as-sex');
});

describe('@pcq @gender', () => {
  test('@axe gender page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await gender.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.genderPageH1, 'i'));
  });

  test('@checklist 6. Page Title is unique', async ({}) => {
    await gender.pageTitleUnique();
  });
