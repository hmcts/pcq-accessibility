...
test.beforeEach(async ({ page }) => {
  preg = new pcqPages(page);
  axe = new AxeUtils(page);
  await preg.invokerSetUp();
  await preg.clickInvoke();
  await page.goto('/pregnant');
});

describe('@pcq @pregnancy', () => {
  test('@axe preg page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await preg.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.pregPageH1, 'i'));
  });
