...
test.beforeEach(async ({ page }) => {
  sex = new pcqPages(page);
  axe = new AxeUtils(page);
  await sex.invokerSetUp();
  await sex.clickInvoke();
  await page.goto('/sex');
});

describe('@pcq @sex', () => {
  test('@axe sex page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await sex.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.sexPageH1, 'i'));
  });
