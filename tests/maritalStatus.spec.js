...
test.beforeEach(async ({ page }) => {
  marital = new pcqPages(page);
  axe = new AxeUtils(page);
  await marital.invokerSetUp();
  await marital.clickInvoke();
  await page.goto('/marital-status');
});

describe('@pcq @maritalStat', () => {
  test('@axe marital page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await marital.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.maritalPageH1, 'i'));
  });
