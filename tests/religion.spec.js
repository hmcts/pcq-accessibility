...
test.beforeEach(async ({ page }) => {
  religion = new pcqPages(page);
  axe = new AxeUtils(page);
  await religion.invokerSetUp();
  await religion.clickInvoke();
  await page.goto('/religion');
});

describe('@pcq @religion', () => {
  test('@axe religion page is accessible', async ({ page }) => {
    await axe.audit(page);
  });

  test('@checklist 1. Headings are visually distinct', async ({}) => {
    await religion.distinctHeaders();
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    expect(await page.title()).toMatch(new RegExp(data.elementNames.religionPageH1, 'i'));
  });
