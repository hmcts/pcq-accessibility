...
class pcqPages {
  ...
  async invokerSetUp() {
    ...
    await this.page
      .getByRole("button", { name: new RegExp(this.elementNames.fill) })
      .click();
    await this.page.waitForSelector(this.locators.serviceIdInput);
    await expect(
      this.page.locator(this.locators.serviceIdInput),
    ).not.toHaveValue("");
    await this.page
      .getByRole("button", {
  ...
  async clickInvoke() {
    ...
    await this.page
      .getByRole("button", { name: new RegExp(this.elementNames.invoke) })
      .click();
  }
  async clickCymraeg() {
  ...
  async distinctHeaders() {
    ...
    if (typeof text === "string" && text !== "") {
      ...
      fontSizet = await pageText.evaluate((el) =>
        parseInt(getComputedStyle(el).fontSize, 10),
      );
    ...
    if (typeof h2Name === "string" && h2Name !== "") {
      ...
      fontSizeh2 = await heading2.evaluate((el) =>
        parseInt(getComputedStyle(el).fontSize, 10),
      );
      heading3 = this.page.getByRole("heading", { name: h3Name, exact: true });
      fontSizeh3 = await heading3.evaluate((el) =>
        parseInt(getComputedStyle(el).fontSize, 10),
      );
